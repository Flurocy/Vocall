  ...
  return (
    <div className="flex h-screen bg-slate-950 text-slate-100">
      <nav className="w-44 border-r border-white/10 p-4">
        ...
        <button onClick={() => setTab('vocab')} className={`mb-2 block w-full rounded-lg px-3 py-2 text-left text-sm ${tab === 'vocab' ? 'bg-emerald-500/20 text-emerald-300' : 'text-slate-300 hover:bg-white/5'}`}>生词库</button>
      ...
      <main className="flex-1 overflow-auto p-6">
        {tab === 'vocab' ? <ExpressionsView /> : <SettingsView />}
      </main>
    </div>
  )
}
