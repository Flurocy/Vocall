...
describe('popup font scale（弹窗内容 zoom 倍率）', () => {
  it('常量范围 0.85–1.4，默认 1.0', () => {
    expect(POPUP_FONT_SCALE_MIN).toBe(0.85)
  ...
  it('正常值原样返回（字符串数字，保留精度）', () => {
    ...
    expect(getPopupFontScale('0.85')).toBe('0.85')
  })
  ...
  it('超范围 clamp 到边界（下界 0.85，上界 1.4）', () => {
    expect(getPopupFontScale('2')).toBe('1.4')
    expect(getPopupFontScale('0.1')).toBe('0.85')
  })
})
